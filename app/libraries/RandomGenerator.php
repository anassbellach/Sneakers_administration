...
class RandomGenerator
{
    ...
    public static function generateRandomNumber($length = 9)
    {
        ...
        $randomNumber = '';
        for ($i = 0; $i < $length; $i++) {
            $randomNumber .= $characters[random_int(0, $charactersLength - 1)];
        }
        return $randomNumber;
    }

    public static function generateRandomBool($length = 1)
    {
        $characters = '01';
        $charactersLength = strlen($characters);
        $randomNumber = '';
    ...
    public static function randomArrays($array, $amount)
    {
        $arrays = (array) $array;

        $randomArray = [];

        foreach (array_rand($arrays, $amount) as $key) {
            $opjectToArray = (array) $arrays[$key];

            $randomArray[] = $opjectToArray;
        }
        return $randomArray;
    }
